...
function EditConfigurationChangeBtn({
  configurations,
  nodes,
  configurationValue,
  configurationChange,
  ...
  const form1 = useForm<EditConfigurationChangeType>({
    resolver: zodResolver(EditConfigurationChangeSchema),
    defaultValues: {
      haveIf: configurationChange.haveIf,
      configurationId: configurationChange.configurationId || undefined,
      checkType: configurationChange.checkType,
      expectedValue: configurationChange.expectedValue,
      change: configurationChange.change.map((change) => ({
      ...
      elseChange: configurationChange.elseChange.map((change) => ({
        nodeId: change.nodeId,
        visible: change.visible,
        changePosition: change.changePosition,
        changeScale: change.changeScale,
  ...
  useEffect(() => {
    ...
    if (configurations && configurationChange.configurationId) {
      ...
      if (configuration) setAvailableValues(configuration.values);
    }
    setHaveIf(configurationChange.haveIf);
  }, [configurationChange]);
...
export default EditConfigurationChangeBtn;
